                ...
                <div class="swiper-wrapper" wire:ignore.self>
                    @foreach($cashouts as $withdrawal)
                        <div class="swiper-slide fade-in-scale card text-white me-2 p-2"
                            ...
                            title="<div class='text-start text-body'><p class='m-0'>Username: {{ $withdrawal->user->username }}</h6> <p class='m-0'>Name: {{ $withdrawal?->user?->privacy ? splitByDashTwoParts($withdrawal->name)['first'] : $withdrawal->name }}</p> <p class='m-0'>Amount: {{ $withdrawal->amount }} Points</p> </div>">
                            <div class="card-body p-0 pb-0 text-center"
