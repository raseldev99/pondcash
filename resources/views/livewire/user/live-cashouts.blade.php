...
<style>
    ...
    .live {
        ...
        animation: pulse 1.5s infinite ease-in-out;
        border-radius: 50%;
        background-color: #00bfff; /* Light blue circle */
    }
</style>
                ...
                <div class="swiper-wrapper" wire:ignore.self>
                    @foreach($cashouts as $withdrawal)
                        <div class="swiper-slide fade-in-scale card text-white me-2 p-2"
                             ...
                             title="<div class='text-start text-body'><p class='m-0'>Username: {{ $withdrawal->user->username }}</h6> <p class='m-0'>Name: {{ $withdrawal->name }}</p> <p class='m-0'>Amount: {{ $withdrawal->amount }} Points</p> </div>">
                            <div class="card-body p-0 pb-0 text-center"
                                 @click="$dispatch('activity-open', {user_id: '{{ $withdrawal->user->id }}'})">
                                <div class="d-flex justify-content-center gap-2">
                                    <div
                                        class="d-flex justify-content-center rounded-circle align-items-center bg-secondary bg-opacity-50  text-white"
                                    ...
                                    <div class="d-flex flex-column text-start align-items-start">
                                        ...
                                        <h6 class="text-secondary pb-0 mb-0">{{ $withdrawal->updated_at->diffForHumans() }}</h6>
                                    </div>
